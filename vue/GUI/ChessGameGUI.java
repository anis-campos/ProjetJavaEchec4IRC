...
public class ChessGameGUI extends AbstractView implements MouseListener, MouseMotionListener {
    ...
    public void mousePressed(MouseEvent e) {
        ...
        // si l'utilisateur a sï¿½lectionnï¿½ une piï¿½ce
        if (c instanceof JLabel) {
            ...
            // avant de dï¿½placer la piï¿½ce, on en positionne un clone invisible
            // au mï¿½me endroit : cela servira lors du rafraichissement de la fenï¿½tre (update)
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        // rï¿½cupï¿½ration du composant qui se trouve ï¿½ position (pixel) finale
        targetSquare = this.damier.findSquareAt(e.getX() - 10, e.getY() - 30);

        // rï¿½cupï¿½ration coordonnï¿½es initiales et finales de la piï¿½ce ï¿½ dï¿½placer
        // en vue du dï¿½placement mï¿½tier
        initCoord = this.damier.getCoord(this.pieceToMoveSquare);
        finalCoord = this.damier.getCoord(targetSquare);

        // Si les coordonnï¿½es finales sont en dehors du damier, on les force ï¿½ -1
        // cela permettra ï¿½ la mï¿½thode chessGame.move de gï¿½rer le message d'erreur
        if (finalCoord == null) {
            finalCoord = new Coord(-1, -1);
        }

        // Invoque la mï¿½thode de dï¿½placement de l'ï¿½chiquier
        this.chessGameControler.move(initCoord, finalCoord);

        // l'ï¿½chiquier est observï¿½ par cette fenï¿½tre
        // dï¿½s lors qu'il est modifiï¿½, la vue en est avertie
        // la mï¿½thode update est appelï¿½e pour rafraichir l'affichage
    }
